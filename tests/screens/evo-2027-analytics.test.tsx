...
describe('Evo 2027 analytics overview', () => {
  ...
  it('opens with the family L1 composition: swipeable month cards and a category section', () => {
    ...
    expect(container.querySelector('[data-evo-analytics-month-bars]')).toBeInTheDocument()
  ...
  it('shows matching top money-out and money-in sections on the overview', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Money out' })).toHaveClass(
      'text-[22px]',
      'font-bold',
      'leading-[28px]',
    )
    expect(screen.getByRole('heading', { name: 'Money in' })).toHaveClass(
      'text-[22px]',
      'font-bold',
      'leading-[28px]',
    )
...
describe('Evo 2027 expense chart and split-by breakdown', () => {
  ...
  it('matches the Figma chart toggle geometry and uses mirrored interval arrows', () => {
    ...
    expect(donutButton).toHaveClass('bg-[var(--uc-surface)]')
    ...
    expect(Array.from(donutButton.querySelectorAll('path')).every((path) => path.getAttribute('fill') === '#666666')).toBe(true)
    ...
    expect(Array.from(barsButton.querySelectorAll('path')).every((path) => path.getAttribute('fill') === '#262626')).toBe(true)

    const previous = screen.getByRole('button', { name: 'Show previous monthly interval' })
    fireEvent.click(previous)
    const next = screen.getByRole('button', { name: 'Show next monthly interval' })
  ...
  it('groups every category after the first three into Other and filters the full breakdown', () => {
    ...
    expect(otherButton).toHaveClass('size-[28px]', 'text-[var(--uc-static-white)]')
    ...
    const otherKeys = breakdownRows()
      ...
      .filter((key): key is string => Boolean(key) && !primaryKeys.includes(key))
