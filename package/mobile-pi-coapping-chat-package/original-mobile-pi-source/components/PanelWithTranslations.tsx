/**
 * Panel Component with Translations
 * 
 * Contains the menu items with Co-Apping support:
 * - ABOUT SMART BANKING
 * - EXCHANGE RATES
 * - FIND ATM & BRANCHES
 * - START CO-APPING SESSION
 */
...
function Frame3({ children }: { children: React.ReactNode }) {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['UniCredit:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[14px] text-[var(--uc-static-white)] w-full whitespace-pre-wrap">{children}</p>
    </div>
...
function Frame7({ children }: { children: React.ReactNode }) {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[16px] items-center min-h-px min-w-px relative">
      <IcNavigationRestyleLocatorWhite1 />
      <Frame3>{children}</Frame3>
    </div>
  );
}

function LightRestyleNavigation3({ startCoAppingSession, onClick }: { startCoAppingSession: string; onClick?: () => void }) {
  return (
    <div 
      className="content-stretch flex gap-[8px] h-[80px] items-center px-[16px] py-[24px] relative shrink-0 w-[375px] cursor-pointer hover:opacity-90 transition-opacity" 
      data-name="Light Restyle/Navigation"
      onClick={onClick}
    >
      <Frame7>{startCoAppingSession}</Frame7>
    </div>
  );
}

function Frame10({ aboutSmartBanking, exchangeRates, findAtmBranches, startCoAppingSession, onStartCoApping }: PanelWithTranslationsProps) {
  return (
    <div className="content-stretch flex flex-col items-center relative shrink-0 w-[375px]">
      ...
      <LightRestyleNavigation3 startCoAppingSession={startCoAppingSession} onClick={onStartCoApping} />
    </div>
  );
}

function Frame8({ aboutSmartBanking, exchangeRates, findAtmBranches, startCoAppingSession, onClose, onStartCoApping }: PanelWithTranslationsProps) {
  return (
    <div className="-translate-x-1/2 absolute bg-[var(--uc-text)] bottom-0 content-stretch flex flex-col gap-[8px] items-start left-1/2 py-[24px] rounded-tl-[12px] rounded-tr-[12px]">
      ...
      <Frame10 aboutSmartBanking={aboutSmartBanking} exchangeRates={exchangeRates} findAtmBranches={findAtmBranches} startCoAppingSession={startCoAppingSession} onStartCoApping={onStartCoApping} />
    </div>
  );
}
