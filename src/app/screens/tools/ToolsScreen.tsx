...
import { SelectionChip, ToolErrorBoundary } from "./toolsUi";
    ...
    <div className="h-full min-h-0 overflow-y-auto bg-[var(--uc-app-bg)]" data-tools-screen="true">
      <div className="mx-auto w-full max-w-[1500px] px-[24px] py-[28px] lg:px-[40px]">
        ...
        <div className="mt-[20px] flex flex-wrap gap-[8px]">
          <SelectionChip active={activeToolId === null} onClick={() => setActiveToolId(null)}>
            Overview
          </SelectionChip>
          {TOOLS.map((tool) => (
            <SelectionChip key={tool.id} active={activeToolId === tool.id} onClick={() => setActiveToolId(tool.id)}>
              {tool.label}
            </SelectionChip>
          ))}
        </div>
        ...
        {activeTool ? (
          <p className="mt-[12px] max-w-[860px] text-[13px] leading-[18px] text-[var(--uc-text-muted)]">
