      ...
      return (
        <>
          ...
          {Array.from({ length: 12 }, (_, index) => {
            ...
            return (
              <circle key={index} cx={18 + Math.sin(angle) * 7} cy={12 - Math.cos(angle) * 7} r="0.9" fill="#FFCC00" />
            )
          })}
        </>
      )
      ...
      return (
        <>
          ...
          {Array.from({ length: 7 }, (_, index) => (
            <rect key={index} y={(index * 24) / 7} width="36" height={24 / 14} fill="#B22234" />
          ))}
    ...
    case 'GBP':
      return (
        <>
          ...
          <path d="M0 0 36 24M36 0 0 24" stroke="#C8102E" strokeWidth="2.5" />
    ...
    case 'CZK':
      return (
        <>
          ...
          <path d="M0 0 16 12 0 24Z" fill="#11457E" />
        </>
      )
    case 'HUF':
      return (
    ...
    case 'RSD':
      return (
        <>
          ...
          <path d="M9 5h6v10l-3 3-3-3Z" fill="#C6363C" stroke="#F4C430" strokeWidth="0.7" />
          <path d="M10.5 9h3M12 7.5v4" stroke="#FFF" strokeWidth="0.8" />
        </>
      )
    case 'BAM':
      return (
        <>
          ...
          <path d="M9 0h18v24Z" fill="#FECB00" />
          {Array.from({ length: 6 }, (_, index) => (
            <circle key={index} cx={7 + index * 4.6} cy={2 + index * 3.8} r="1.15" fill="#FFF" />
          ))}
        </>
      )
  }
...
function RoundFlagArtwork({ currency }: CurrencyFlagProps) {
  if (currency === 'EUR') {
    return (
      <>
        ...
        {Array.from({ length: 12 }, (_, index) => {
          ...
          return (
            <circle
              key={index}
              cx={18 + Math.sin(angle) * 10.5}
              cy={18 - Math.cos(angle) * 10.5}
              r="1.35"
              fill="#FFCC00"
            />
          )
  ...
  if (currency === 'USD') {
    const stripeHeight = 36 / 13
    return (
      <>
        <rect width="36" height="36" fill="#FFF" />
        {Array.from({ length: 7 }, (_, index) => (
          <rect key={index} y={index * stripeHeight * 2} width="36" height={stripeHeight} fill="#B22234" />
        ))}
        <rect width="18" height={stripeHeight * 7} fill="#3C3B6E" />
        {Array.from({ length: 20 }, (_, index) => (
          <circle
            key={index}
            cx={2.3 + (index % 5) * 3.25}
            cy={2.2 + Math.floor(index / 5) * 3.8}
            r="0.72"
            fill="#FFF"
          />
        ))}
  ...
  return (
    ...
    >
      <svg
        ...
        focusable="false"
      >
        <RoundFlagArtwork currency={currency} />
      </svg>
    </span>
  )
}
