# UniCredit Build UI Bridge

Local Figma development plugin for bidirectional Mobile Banking CEE screen handoff.
...
2. Select `figma-plugins/screen-json-importer/manifest.json`.
3. Run `UniCredit Build UI Bridge`.
...
The same bridge has also been synced into `screenshots/FIgma plugins/Component-E` as `Component-E Build UI Bridge`, preserving the original Component-E development plugin id while adding the Build-UI import capability.
...
```bash
node --check figma-plugins/screen-json-importer/code.js
