...
| todo | Add automated tests for reconstructed template previews | Browser smoke confirmed 50 cards, 50 code-backed templates, 30 screenshot sources, 20 code-only templates, default `code` mode, source toggle only for screenshot-backed templates, compact `168px` cards, and selection for reconstructed/code-only template packs; no automated regression test exists yet |
