...
| done | Reconstruct remaining source-only screenshot templates as reusable code previews | Current code-backed coverage is 30/30; the screenshot files remain source comparison evidence and no template is source-only in the registry |
| todo | Add automated tests for reconstructed template previews | Browser smoke confirmed 30 cards, 30 code-backed templates, default `code` mode, source toggle, compact `190px` cards, and selection for reconstructed template packs; no automated regression test exists yet |
