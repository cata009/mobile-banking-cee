...
- Expanded evolution product cards now use an explicit `0.5px` neutral separator between adjacent cards. The reusable `ProductCard` also reserves a real `64x40` leading-art slot for card artwork, keeping the artwork inside the card's existing right inset instead of overflowing into the edge.
- Downloaded and committed the requested Figma source photography from Transformation 1 (`23968:242772`) and Transformation 2 (`23968:245630`). For your interest has three real image-backed cards on every tab; ShopSmart has image-backed offers instead of gradient placeholders.
- Added a focused regression covering compact product headers, card separators, card-art sizing, and interest/ShopSmart campaign media. `tests/screens/app-2027-homepage.test.tsx` passes (`5/5`); `npm run typecheck`, targeted ESLint, and `npm run build` pass.
- In-app Browser visual QA on the local light iPhone 17 Pro Max preview confirmed the compact Account/Card header structure, visible inter-card separators, card art inside the right gutter, non-empty media element counts (`3` interest / `2` ShopSmart), no error overlay, and no console errors. The temporary QA tab was not retained; the user's live Evo tab remains open.
- Follow-up corrections: Evo now activates the same App 2027 glass `BottomNavigation` on Spending, Payments, Products, and More (not only Home). `Your recent transactions` has its heading outside the white activity surface; ShopSmart filters are real selectable buttons; the Accounts summary reuses the existing Baseline lighthouse asset while retaining `Total Available` / `Spent this week`; Saving Account cards use the account-card hierarchy without quick actions; and supporting maturity/installment values render at `16px` rather than product-total scale.
- Fresh focused regression coverage now passes (`7/7`), alongside `npm run typecheck`, targeted ESLint, and `npm run build`. Live preview evidence: Figma-interest photos have natural widths `480/1988/1989px`, ShopSmart photography `3540/1989px`, all summary art is loaded, Credits has only the compact `Credit Cards` header with `13px` artwork inset, and the `E-shops` selector updates its active state and offer content. Light iPhone, dark iPhone, and Galaxy Z Fold8 closed were inspected with no error overlay or horizontal overflow. Spending was opened on the Fold preview and uses the same glass navigation with its active state.
- Insurance follows the same compact expanded-list treatment as Accounts: policy rows now share one rounded surface with a `0.5px` divider, instead of separated cards. The Insurance banner's active-policy count is explicitly `14px` / `18px` to match the supporting summary typography. Focused regression passes (`8/8`), `npm run typecheck`, and targeted ESLint pass. Live Insurance-preview inspection confirms two visible policy rows, `0px` inter-row gap, `0.5px` top divider for the second policy, a computed `14px` count, and no error overlay.
- Evo CZ Saving Account now exposes `Add money` as action three in its detail action bar. It opens a scoped Kids-HU-inspired amount flow with CZK number pad, selected real current-account funding source, presets, source picker, balance guard, and optional recurring standing-order sheet (`Start date`, `Repeat`, `End`). Its success states remain session-only demonstration states; management/listing of a created standing order is deliberately deferred until its destination is specified. Focused `saving-account-add-money` coverage passes (`1/1`), plus `npm run typecheck` and targeted ESLint. Live preview confirmed the active `sav-1` action opens the flow with `Everyday account · 22 850,50 CZK`, a ten-key keypad and no error overlay; the schedule sheet exposes all three requested controls.
- Final EVO interaction follow-up: the Accounts group now keeps the lead card open, leaves the next card as a compact peek, and expands all remaining cards through either the group header or the peek. The device control is a compact current-device icon with a real dropdown menu; each foldable profile has explicit no-notch chrome, while Dynamic Island stays only on the iPhone profile in both orientations.
- Final verification before publication: focused device/Home tests pass (`11/11`); `npm run typecheck`, `npm run lint`, and `npm run build` pass. Browser visual QA on local Evo Home confirmed collapsed Accounts (`1` full card + preview) and expanded Accounts (`3` cards), compact selector with no native select, iPhone Dynamic Island present, and Apple Passport closed/rotated with no Dynamic Island.
- Full `npm test -- --run`: `760/762` tests pass. Excluded from this Evo closeout: PFM recategorization expects `RON` while its existing fixture renders `CZK`; Analytics template-preview checksum has a stale expected hash; concurrent untracked `business-requirements-registry.test.mjs` has no Vitest suite. Those three are not part of the Evo implementation and are triaged below.
- The five source/registry audits pass. The sixth, `audit:assets`, initially found a stale 182-asset baseline despite six already-approved Evo images and four new referenced campaign images; it was re-locked to the Git-canonical 192-asset aggregate without deleting, replacing, or optimizing any asset.
- Publication: product commit `3368fa1` is pushed to `origin/main`. Vercel production deployment `dpl_8b7HpMXSwPA5ZuQdUdSsgkZittP1` is `READY` and aliased to `https://mobile-banking-cee.vercel.app`; both the deployment URL and canonical alias returned HTTP `200` with `Cache-Control: no-store`.
- Limitation: the source Figma photographs are intentionally committed at their supplied dimensions for fidelity; optimize their encoded size in a dedicated performance pass without replacing or visually degrading the approved images.
- Banana Loop: scoped staging excludes concurrent HU Kids code, the business-requirements registry HTML/test, and `work/`. Full-suite blockers are recorded in `known-bananas.md` and `next-tasks.md`; no Evo data or standing order persists beyond the demo session.
- constitutional check:
  - scope preserved: yes
  - docs updated: yes
  - verification recorded: yes
  - bananas triaged: yes
  - safe to resume: yes; Evo changes have focused, build, type, lint, full-suite and browser evidence, with only unrelated triaged blockers remaining.
