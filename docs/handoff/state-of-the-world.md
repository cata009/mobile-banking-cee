...
- `Mobile PI Kids` now renders a Romania-only, current-design-system RO Kids prototype with deep Kid/Parent mock flows; Czech Republic, Slovakia, Hungary, Bosnia, Bosnia Banja Luka, and Slovenia additionally render current-design-system homepage plus bottom-navigation concept variants for comparison. Slovakia is now a Bulbank document-inspired concept with Products, Education, Tasks, and More pages. Serbia and future-design-system Kids contexts still render the honest planned-state placeholder until separate concepts are implemented;
...
- CZ/SK/HU/BA/BA_BL/SI Kids concept variants for the first concept-battle pass: Czech Republic uses a pocket-plan homepage, Slovakia uses a Bulbank document-inspired Products/Education/Tasks/More experience, Hungary uses a smarter fintech-inspired cockpit with pockets/insights, Bosnia uses a family approval hub, Bosnia Banja Luka uses a card-first variant, and Slovenia uses a weekly goal-coach variant while staying inside UniCredit tokens, existing shell, `AppIcon`, `Card`, `ProfileAvatar`, and shared section patterns.
...
- a full multi-country Kids implementation; RO is the only deep Kid/Parent mock-flow prototype, CZ/SK/HU/BA/BA_BL/SI are mock-driven homepage/navigation concept variants only, and RS Kids remains a planned placeholder;
