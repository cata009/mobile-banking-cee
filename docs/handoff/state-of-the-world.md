...
- a Vercel-facing demo access gate: unauthenticated production requests are redirected by root `middleware.js` to `public/access.html`, successful password entry is validated by `api/access.js`, and access is represented by signed HTTP-only cookies. `Remember my password` keeps the access cookie for 6 months; unchecked access keeps it for 1 month. Failed attempts are counted in a signed HTTP-only cookie and the 10th failed attempt triggers a temporary 24-hour block with support-only copy. The Vite local dev server still uses the React `AccessGate` fallback because Vite does not execute Vercel middleware/functions.
